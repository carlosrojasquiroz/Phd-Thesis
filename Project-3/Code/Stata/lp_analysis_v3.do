*-------------------------------------------------------------------------------
* Section 5: Local projection analysis (interaction with fixed err)
*-------------------------------------------------------------------------------
...
foreach var of local varendo {
	...
	* Loop over horizons
	quietly forvalues h = 0/$HOR {	
		xtscc y_`h' sl_ sh_ slexrr_ shexrr_ id_low id_high L(1/ $LAG ).(dyl_ sl_ dyh_ sh_) F`h'.(dum1-dum11) i.code if quarter >= tq(1998q1),lag(`h') noconstant
		replace bl = $SIGN *( _b[sl_]+ _b[slexrr_] ) if _n == `h'+1
		replace ul90 = $SIGN *( _b[sl_]+ _b[slexrr_] ) + $T90 * ( _se[sl_]^2 + _se[slexrr]^2 )^0.5  if _n == `h'+1
		replace ll90 = $SIGN *( _b[sl_]+ _b[slexrr_] ) - $T90 * ( _se[sl_]^2 + _se[slexrr]^2 )^0.5  if _n == `h'+1
		replace ul68 = $SIGN *( _b[sl_]+ _b[slexrr_] ) + $T68 * ( _se[sl_]^2 + _se[slexrr]^2 )^0.5  if _n == `h'+1
		replace ll68 = $SIGN *( _b[sl_]+ _b[slexrr_] ) - $T68 * ( _se[sl_]^2 + _se[slexrr]^2 )^0.5  if _n == `h'+1
		replace bh = $SIGN *( _b[sh_]+ _b[shexrr_] ) if _n == `h'+1
		replace uh90 = $SIGN *( _b[sh_]+ _b[shexrr_] ) + $T90 * ( _se[sh_]^2 + _se[shexrr]^2 )^0.5  if _n == `h'+1
		replace lh90 = $SIGN *( _b[sh_]+ _b[shexrr_] ) - $T90 * ( _se[sh_]^2 + _se[shexrr]^2 )^0.5  if _n == `h'+1
		replace uh68 = $SIGN *( _b[sh_]+ _b[shexrr_] ) + $T68 * ( _se[sh_]^2 + _se[shexrr]^2 )^0.5  if _n == `h'+1
		replace lh68 = $SIGN *( _b[sh_]+ _b[shexrr_] ) - $T68 * ( _se[sh_]^2 + _se[shexrr]^2 )^0.5  if _n == `h'+1
		lincom $SIGN *(sh_ + shexrr_ - sl_ - slexrr_)
	...
	nois esttab, se nocons keep(sl_ slexrr_ sh_ shexrr_) ti("`varlab' - Sample: 1998Q1-2023Q3") label star(* 0.32 ** 0.10) t(3) nogaps b($FORMATT)
	*---------------------------------------------------------------------------
	...
	* Save graph file
	quietly graph save "$dir/Results/Figures/GPH/`var'low_$VERSION.gph", replace	
	quietly graph export "$dir/Results/Figures/PDF/`var'low_$VERSION.pdf", replace
	*---------------------------------------------------------------------------
	...
	* Save graph file
	quietly graph save "$dir/Results/Figures/GPH/`var'high_$VERSION.gph", replace	
	quietly graph export "$dir/Results/Figures/PDF/`var'high_$VERSION.pdf", replace
	*---------------------------------------------------------------------------
	...
	* Save graph file
	quietly graph save "$dir/Results/Figures/GPH/`var'test_$VERSION.gph", replace	
	quietly graph export "$dir/Results/Figures/PDF/`var'test_$VERSION.pdf", replace		
